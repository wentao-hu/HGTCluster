...
class EgoHGTDataLoader:
    ...
    def _query(self, graph, relation):
        ...
        if src_node_type == dst_node_type:
            neg_src = src.outNeg(relation).sample(self._neg_num).by(self._sampler).alias(
                'neg_src_node_' + src_node_type)
            neg_dst = dst.outNeg(relation).sample(self._neg_num).by(self._sampler).alias(
                'neg_dst_node_' + dst_node_type)
        else:
            neg_src = dst.inNeg(relation).sample(self._neg_num).by(self._sampler).alias('neg_src_node_' + src_node_type)
            neg_dst = src.outNeg(relation).sample(self._neg_num).by(self._sampler).alias(
                'neg_dst_node_' + dst_node_type)
