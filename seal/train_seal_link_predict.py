...
def train(g, model, predictor, config):
  ...
  src = seed.outV().alias('pos_src')  # get source nodes of edges
  src.outV('train').sample(config['nbrs_num']).by('full').alias('src_hop1')

  dst = seed.inV().alias('pos_dst') # get dst nodes of edges
  dst.outV('train').sample(config['nbrs_num']).by('full').alias('dst_hop1')

  src.outNeg('train').sample(1).by('random').alias('neg_dst').\
    outV('train').sample(config['nbrs_num']).by('full').alias('neg_hop1')  # negative sampling
...
def run(config):
  ...
  # trainer
  iterator, loss = train(g, model, predictor, config)  # iterator is dataset.iterator
  ...
  with tf.Session() as sess:
    ...
    for step in range(config['steps']):
      try:
        ...
        if step % 1000 == 0: # test every 1000 steps
...
if __name__ == "__main__":
  config = {'dataset_folder': '../data/ogbl_collab/',
            ...
            'drop_out': 0.0,
            'predictor_layers': 3,
            'steps': 2000,  # equal to train epochs
