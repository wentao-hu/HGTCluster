...
follow_note_path = os.path.abspath(os.path.join(current_path,'..'))+'/data/local/fake_follow_note'
if not os.path.exists(follow_note_path):
    os.makedirs(follow_note_path)


with open('{}/fake_follow_note.csv'.format(follow_note_path),"w") as pos_f:
    pos_f.write('src_id:int64'+'\t'+'dst_id:int64'+'\t'+'weight:float'+'\t'+'label:int32'+'\n')
with open('{}/fake_follow_note.csv'.format(follow_note_path),"a") as pos_f:
    for line in pos_follow_note_list:
...
# %%
current_path = sys.path[0]
share_note_path = os.path.abspath(os.path.join(current_path,'..'))+'/data/local/fake_share_note'
if not os.path.exists(share_note_path):
    os.makedirs(share_note_path)
    

with open('{}/fake_share_note.csv'.format(share_note_path),"w") as pos_f:
    pos_f.write('src_id:int64'+'\t'+'dst_id:int64'+'\t'+'weight:float'+'\t'+'label:int32'+'\n')
with open('{}/fake_share_note.csv'.format(share_note_path),"a") as pos_f:
    for line in pos_share_note_list:
        pos_f.write(str(line[0])+'\t'+str(line[1])+'\t'+'1.0'+'\t'+'1')
        pos_f.write('\n')
