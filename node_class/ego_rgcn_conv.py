...
class EgoRGCNConv(EgoConv):
  ...
  def __init__(self,
    ...
    if isinstance(in_dim, list) or isinstance(in_dim, tuple):
      ...
      self._in_dim = [in_dim, in_dim] # if in_dim is an integer, source and neighbor share the same dimension.
  ...
  def forward(self, x, neighbor, expand):
    ...
    if self._num_blocks is not None:
      ...
      for i in range(self._num_relations):
        h = agg_func(neighbor[i], expand, self._in_dim[1])  # n * input_dim[1]
        h = tf.matmul(h, weight[i])     # n * input_dim[1]  input_dim[1] * out_dim  -> n * out_dim
        out.append(h)  
    out = tf.math.add_n(out)  # bs * out_dim

    out += self.root_weight(x)
    if self.bias is not None:
      out += self.bias
    return out    # bs * out_dim
