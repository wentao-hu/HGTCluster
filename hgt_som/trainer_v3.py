...
class DistTrainer(object):
  ...
  def train(self, iterator, loss, learning_rate, epochs=10, hooks=[], metrics=[], var_list=None, global_step=None, cluster_ops=None, **kwargs):
    with self.context():
      ...
      while epoch < epochs:       
        try:
          if self.profiling and self.task_index == 1 and \
            ...
            cluster_outs = self.sess.run(cluster_ops)
            ...
            cluster_outs = self.sess.run(cluster_ops)  # first update cluster center embedding
            outs = self.sess.run(train_ops)  # then use optimizer.minimize() to update HGT parameters
        ...
        if local_step % 5 == 0:
          print(datetime.datetime.now(),
                'Epoch {}, Global_step {}, Iter {}, Global_step/sec {:.2f}, Time(s) {:.4f}, Loss {:.5f}'
                .format(epoch, global_step, local_step,
                        (global_step - last_global_step) * 1.0 / (time.time() - t),
                        (time.time() - t) * 1.0 , train_loss))
          print('decay_lr {:.5f}, decay_radius {:.5f}, cluster_norm {:.5f} '
                .format(cluster_outs[1], cluster_outs[2], cluster_outs[3])) 
          t = time.time()
