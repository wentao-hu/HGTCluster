...
from ego_hgt_link_dataloader import EgoHGTDataLoader
...
def init_graph(TRAINING_DATA_NODE_LIST, TRAINING_DATA_EDGE_LIST):
  ...
  author_decoder = gl.Decoder(labeled=False, attr_types=author_attr_types, attr_dims=author_attr_dims, attr_delimiter=":")
  ...
  g = gl.Graph()\
        ...
        .node(TRAINING_DATA_NODE_LIST[1], node_type='author',
          decoder=author_decoder)                      \
        ...
        .edge(TRAINING_DATA_EDGE_LIST[1], edge_type=('user','note', 'pos_commentat_note'),
              decoder=gl.Decoder(weighted=True, labeled=True), directed=False) \
        .edge(TRAINING_DATA_EDGE_LIST[2], edge_type=('user','user', 'pos_commentat_user'),
              decoder=gl.Decoder(weighted=True, labeled=True), directed=False) \
        .edge(TRAINING_DATA_EDGE_LIST[3], edge_type=('user','user', 'pos_return_to_user'),
              decoder=gl.Decoder(weighted=True, labeled=True), directed=False) \
        .edge(TRAINING_DATA_EDGE_LIST[4], edge_type=('user','note', 'pos_follow_note'),
              decoder=gl.Decoder(weighted=True, labeled=True), directed=False) \
        .edge(TRAINING_DATA_EDGE_LIST[5], edge_type=('user','author', 'pos_follow_user'),
              decoder=gl.Decoder(weighted=True, labeled=True), directed=False) \
        .edge(TRAINING_DATA_EDGE_LIST[6], edge_type=('user','note', 'pos_share_note'),
        decoder=gl.Decoder(weighted=True, labeled=True), directed=False) \
        .edge(TRAINING_DATA_EDGE_LIST[7], edge_type=('user','note', 'pos_return_to_note'),
              decoder=gl.Decoder(weighted=True, labeled=True), directed=False)           
...
def train():
  ...
  if job_name == "graphlearn":
    ...
    with trainer.context():
      ...
      author_feature_handler = tfg.FeatureHandler('author_feature_handler',decoder_dict['author_decoder'].feature_spec)
      ...
      num_pos_neg_pairs =  len(pos_relation_dict) # len(pos_relation_dict)
      for i in range(len(pos_relation_dict)):  #should be len(pos_relation_dict) for all training 
        ...
        neg_relation = neg_relation_dict.keys()[i]
        print('===Start sampling for:','pos_relation:',pos_relation,'neg_relation:',neg_relation)      
        
        train_data = EgoHGTDataLoader(g, gl.Mask.TRAIN, FLAGS.sampler, FLAGS.batch_size, 
                                    pos_relation=pos_relation, neg_relation=neg_relation, 
                                    pos_relation_dict = pos_relation_dict, neg_relation_dict=neg_relation_dict, 
                                    user_feature_handler=user_feature_handler, author_feature_handler=author_feature_handler, note_feature_handler=note_feature_handler,
                                    nbrs_num=nbrs_num)
        # pos_iterator and neg_iterator
        pos_iterator = train_data._pos_iterator
        neg_iterator = train_data._neg_iterator
        iterator_list.append(pos_iterator)
        iterator_list.append(neg_iterator)

        ## get neighborhood features 
        pos_src_x_list, pos_dst_x_list = train_data.x_list(train_data._q_pos, train_data._pos_data_dict, train_data._pos_alias_dict, train_data._pos_node_type_path_dict)
        neg_src_x_list, neg_dst_x_list = train_data.x_list(train_data._q_neg, train_data._neg_data_dict, train_data._neg_alias_dict, train_data._neg_node_type_path_dict)
        
        pos_src_relation = train_data._pos_relation_path_dict['src']
        pos_dst_relation = train_data._pos_relation_path_dict['dst']
        neg_src_relation = train_data._neg_relation_path_dict['src']
        neg_dst_relation = train_data._neg_relation_path_dict['dst']

        pos_src_node_type = train_data._pos_node_type_path_dict['src']
        pos_dst_node_type = train_data._pos_node_type_path_dict['dst']
        neg_src_node_type = train_data._pos_node_type_path_dict['src']
        neg_dst_node_type = train_data._pos_node_type_path_dict['dst']
        
        pos_src_relation_count = train_data._pos_relation_count_dict['src']
        pos_dst_relation_count = train_data._pos_relation_count_dict['dst']
        neg_src_relation_count = train_data._neg_relation_count_dict['src']
        neg_dst_relation_count = train_data._neg_relation_count_dict['dst']

        pos_src_embedding = model.forward(pos_src_x_list, pos_src_relation, pos_src_node_type, pos_src_relation_count, nbrs_num)
        pos_dst_embedding = model.forward(pos_dst_x_list, pos_dst_relation, pos_dst_node_type, pos_dst_relation_count, nbrs_num)
        neg_src_embedding = model.forward(neg_src_x_list, neg_src_relation, neg_src_node_type, neg_src_relation_count, nbrs_num)
        neg_dst_embedding = model.forward(neg_dst_x_list, neg_dst_relation, neg_dst_node_type, neg_dst_relation_count, nbrs_num)
        ...
        accuracy_train, update_op_train = tuple( 0.5*a + 0.5*b for a, b in zip(build_acc(pos_src_embedding, pos_dst_embedding, neg_dst_embedding), build_acc(pos_dst_embedding, pos_src_embedding, neg_src_embedding)))
        loss = 0.5*build_bpr_loss(pos_src_embedding, pos_dst_embedding, neg_dst_embedding) + 0.5*build_bpr_loss(pos_dst_embedding, pos_src_embedding, neg_src_embedding)
      ...
      user_x_list = user_dataset.x_list(user_dataset._q, user_dataset._data_dict, user_dataset._alias_list, user_dataset._node_type_path)
      user_embedding = model.forward(user_x_list, user_dataset._relation_path, user_dataset._node_type_path, user_dataset._relation_count, nbrs_num)
      ...
      author_x_list = author_dataset.x_list(author_dataset._q, author_dataset._data_dict, author_dataset._alias_list, author_dataset._node_type_path)
      author_embedding = model.forward(author_x_list, author_dataset._relation_path, author_dataset._node_type_path, author_dataset._relation_count, nbrs_num)
      ...
      note_x_list = note_dataset.x_list(note_dataset._q, note_dataset._data_dict, note_dataset._alias_list, note_dataset._node_type_path)
      note_embedding = model.forward(note_x_list, note_dataset._relation_path, note_dataset._node_type_path, note_dataset._relation_count, nbrs_num)
    ...
    trainer.train(iterator_list, total_loss, FLAGS.learning_rate, epochs=FLAGS.train_epochs, metrics=[avg_acc,avg_update_op])
...
def define_custom_flags():
  ...
  # for data path
  flags.DEFINE_string(name='train_data_base_dir', default=None, help='Training data base dir') # /root/data/topic_graph/data/v2_rename/
  ...
  flags.DEFINE_integer('neg_num', 1, 'number of negative samples for each src/dst node in pos relation')
  # if we use self-defined neg edges, neg_num = neg batch size: pos batch size, here is 1


  # for load and save model
  flags.DEFINE_string(name='load_model_dir', default="", help='Model dir for load before training') # /root/data/hgt/model/20230722/
...
if __name__ == "__main__":
  ...
  print('====flags====')
  print('save emb dir is',FLAGS.save_emb_dir)
  print('save model dir is',FLAGS.save_model_dir)
  print('save emb dir',FLAGS.save_emb_dir)
  ...
  neg_relation_dict = OrderedDict()
  neg_relation_dict['pos_comment_note']=['user','note'] 
  neg_relation_dict['pos_commentat_note']=['user','note']
  neg_relation_dict['pos_commentat_user']=['user','user']
  neg_relation_dict['pos_return_to_user']=['user','user']
  neg_relation_dict['pos_follow_note']=['user','note']
  neg_relation_dict['pos_follow_user']=['user','author']
  neg_relation_dict['pos_share_note']=['user','note']
  neg_relation_dict['pos_return_to_note']=['user','note'] 
