...
class DistTrainer(object):
    ...
    def save_node_embedding_list(self, emb_path, iterator_list, ids, emb):
        ...
        with self.context():
            ...
            last_local_step = 0
            t = time.time()
            # TODO: one relation outofrange then HGT/link_pred/trainer_saver.pyend
            self.sess._tf_sess().run([iterator.initializer for iterator in iterator_list])
            emb_dict = {}
            print('load model global step = ' + str(self.sess._tf_sess().run(self.global_step)))
            while True:
                try:
                    ...
                    if local_step <= 20 or local_step % 1000 == 0:
                        print(datetime.datetime.now(),
                              'Iter {}, local_step/sec {:.2f}, Time(s) {:.4f}'
                              .format(local_step,
                                      (local_step - last_local_step) / (time.time() - t),
                                      (time.time() - t) * 1.0))
                        t = time.time()
                        last_local_step = local_step
                except tf.errors.OutOfRangeError:
                    print("total local_step = {}".format(local_step))
                    print("Start write to file...")
                    print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()))
                    print("total embs count = {}".format(len(emb_dict)))
